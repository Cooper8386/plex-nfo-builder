...
- **Library sorting** (v0.13.0): a Sort dropdown on every library toolbar — Title (A-Z), Title (Z-A), Date Added, Date Updated, and Season Count (On disk). The choice is remembered per library. Date Added is recorded when the scanner first sees a folder (backfilled from folder mtime on upgrade); Date Updated tracks the newest mtime across the item folder and its season subfolders, so a new episode landing in `Season NN/` bumps the show.
